...
> 提交说明：本文档已按当前项目状态预填，可直接复制到作业提交模板或飞书文档中。作者姓名、演示视频链接可在提交前按实际情况修改。
...
- 演示视频：建议录制 3-5 分钟，覆盖登录 -> 素材/Prompt -> AI 生成 -> 审核 -> 发布 -> 详情反馈 -> 榜单 -> 模拟抖音分发；脚本见 `docs/demo-video-script.md`。如果平台未强制，可填写“未提供，线上环境可直接体验完整链路”。
...
- 抖音图文沙盒适配层已接入，发布内容可生成模拟外部作品 ID；真实抖音/头条开放 API 待具备开放平台资质后替换为 live adapter。
...
- 完整项目代码压缩包：提交附件使用 `deliverables/toutiao-ai-creator-platform-source.zip`；也可从 GitHub master 下载源码压缩包。
...
- 在具备资质后把当前抖音图文沙盒适配层替换为抖音/头条开放平台 live adapter，完成真实热点获取和一键分发。
