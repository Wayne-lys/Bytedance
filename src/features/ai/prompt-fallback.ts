...
export function buildPromptFallbackDraft(
  ...
  if (mode === "headline") {
    ...
    return {
      ...
      body: `${materialText}可从下面 10 个标题里选一个继续扩写：\n${numberedLines(headlines)}`,
      tags: [...baseTags, "标题优化"],
      coverSuggestion: input.materials[0] ?? "使用主体明确、文字留白充足的封面图，方便叠加标题。",
  ...
  if (mode === "checklist") {
    ...
    return {
      ...
      body: `${materialText}${input.audience}可以按这个清单执行：\n${numberedLines(checklist)}`,
      tags: [...baseTags, "内容清单"],
      coverSuggestion: input.materials[0] ?? "使用干净的列表感封面，突出步骤和要点。",
  ...
  if (mode === "feed") {
    return {
      ...
      body: `${materialText}${input.topic}最常见的场景，是忙碌时想快速做出一个不出错的选择。对${input.audience}来说，重点不是准备很多方案，而是把判断标准变简单。\n\n第一，先固定一个默认选择。默认选择能减少临时纠结，也更容易观察效果。\n\n第二，把“舒服”和“可持续”放在前面。短期看起来更强的方案，如果执行成本高，往往坚持不了。\n\n第三，给自己留一个复盘点。比如一周后看精神状态、花费和便利度，再决定要不要调整。`,
      tags: [...baseTags, "信息流"],
      coverSuggestion: input.materials[0] ?? "使用明亮、干净、主体明确的信息流封面图。",
  ...
  return {
    ...
    body: `${materialText}${input.topic}这类内容适合先给出一个具体场景，再把建议拆成可执行步骤。面向${input.audience}时，表达要保持${input.style}，不要只停留在概念层面。\n\n可以先写问题：什么时候会遇到这个选择？再写方法：具体怎么做？最后写提醒：哪些地方容易被忽略？这样读者能直接判断自己是否适用。`,
    tags: [...baseTags, "自定义模板"],
    coverSuggestion: input.materials[0] ?? "使用与选题直接相关、主体清晰的封面图。",
